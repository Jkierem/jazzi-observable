...
describe("Observable", () => {
  ...
  describe("constructors", () => {
    it("should construct from function", () => {
      ...
      } = createMockObservable(() => Observable.from<number>((sub) => {
  ...
  describe("methods", () => {
    describe("Observable", () => {
      ...
      it("sequence -> unsubscribe should unsub from both sequenced observables", (done) => {
        const first = Observable.fromArray([1,2,3])
        const second = Observable.from<number>(sub => Promise.resolve().then(() => {
      ...
      it("catchError -> should subscribe to new observable, calling both unsubscribes", () => {
        expect.assertions(3);
        const unsubSpy = Spy();
        const original = Observable.from<unknown, number>((sub) => {
          sub.error(42);
          return unsubSpy
        });
        let thrown, other;
